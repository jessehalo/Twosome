...
\
const CATEGORIES = [\
  \{ label: "Food", icon: "\uc0\u55356 \u57213 \u65039 " \},\
  \{ label: "Groceries", icon: "\uc0\u55357 \u57042 " \},\
  \{ label: "Transport", icon: "\uc0\u55357 \u56983 " \},\
  \{ label: "Home", icon: "\uc0\u55356 \u57312 " \},\
  \{ label: "Entertainment", icon: "\uc0\u55356 \u57260 " \},\
  \{ label: "Travel", icon: "\uc0\u9992 \u65039 " \},\
  \{ label: "Health", icon: "\uc0\u55357 \u56458 " \},\
  \{ label: "Other", icon: "\uc0\u55357 \u56550 " \},\
];\
\
...
\
  const saveExpense = async () => \{\
    ...
    if (editingId) \{\
      ...
      showToast("Expense updated \uc0\u10003 ");\
    \} else \{\
      await supabase.from("expenses").insert(payload);\
      showToast("Expense added \uc0\u10003 ");\
    \}\
...
\
  const settleUp = async () => \{\
    ...
    setSettling(false);\
    showToast("Settled up! Balance cleared \uc0\u10003 ");\
  \};\
\
...
\
  const saveSettings = async () => \{\
    ...
    showToast("Settings saved \uc0\u10003 ");\
  \};\
\
  const catIcon = (label) => CATEGORIES.find((c) => c.label === label)?.icon || "\uc0\u55357 \u56550 ";\
    ...
    <div style=\{S.authRoot\}>\
      ...
      <div style=\{S.authCard\}>\
        ...
        <div style=\{S.formGroup\}>\
          ...
          <input style=\{S.input\} type="password" placeholder="\'95\'95\'95\'95\'95\'95\'95\'95" value=\{authForm.password\} onChange=\{(e) => setAuthForm(\{ ...authForm, password: e.target.value \})\} onKeyDown=\{(e) => e.key === "Enter" && handleAuth()\} />\
        </div>\
        ...
        \{onboarding === "create" && (\
          <>\
            ...
            <div style=\{S.formGroup\}>\
              ...
              <input style=\{S.input\} placeholder="e.g. Josh & Kristy" value=\{householdName\} onChange=\{(e) => setHouseholdName(e.target.value)\} onKeyDown=\{(e) => e.key === "Enter" && createHousehold()\} />\
            </div>\
            <button onClick=\{createHousehold\} style=\{\{ ...S.addBtn, opacity: householdName.trim() ? 1 : 0.4 \}\}>Create</button>\
            <button onClick=\{() => setOnboarding("choice")\} style=\{\{ ...S.authSwitchBtn, display: "block", margin: "16px auto 0" \}\}>\uc0\u8592  Back</button>\
          </>\
        )\}\
        \{onboarding === "join" && (\
          <>\
    ...
    <div style=\{S.root\}>\
      <style>\{`\
        ...
        * \{ box-sizing: border-box; margin: 0; padding: 0; \}\
        ...
        .er \{ transition: background 0.15s; \}\
        .er:hover \{ background: rgba(128,128,128,0.08) !important; \}\
        .ib \{ opacity: 0; transition: opacity 0.15s; \}\
        .er:hover .ib \{ opacity: 1; \}\
        @keyframes su \{ from \{ transform: translateY(20px); opacity: 0; \} to \{ transform: translateY(0); opacity: 1; \} \}\
        @keyframes fi \{ from \{ opacity: 0; \} to \{ opacity: 1; \} \}\
        @keyframes ti \{ from \{ transform: translateY(10px) translateX(-50%); opacity: 0; \} to \{ transform: translateY(0) translateX(-50%); opacity: 1; \} \}\
        @keyframes mi \{ from \{ transform: translateY(30px); opacity: 0; \} to \{ transform: translateY(0); opacity: 1; \} \}\
        .slideup \{ animation: su 0.35s ease forwards; \}\
        .fadein \{ animation: fi 0.3s ease forwards; \}\
        .toast \{ animation: ti 0.3s ease forwards; \}\
        .modalin \{ animation: mi 0.3s ease forwards; \}\
        .cswatch \{ transition: transform 0.15s; cursor: pointer; \}\
              ...
              <div style=\{S.toggleRow\}>\
                \{["dark", "light", "colorful"].map((t) => (\
                  <button key=\{t\} className="ab" onClick=\{() => setSettingsForm(\{ ...settingsForm, theme: t \})\}\
                    ...
                    \{t === "dark" ? "\uc0\u55356 \u57113  Dark" : t === "light" ? "\u9728 \u65039  Light" : "\u55356 \u57256  Color"\}\
                  </button>\
                ))\}\
              </div>\
            </div>\
\
            <div style=\{S.formGroup\}>\
              ...
              <div style=\{S.inviteBox\}>\
                ...
                <div style=\{S.inviteCode\}>\{household?.invite_code\}</div>\
              </div>\
            </div>\
\
      ...
      <div style=\{S.header\}>\
        <div style=\{S.headerInner\}>\
          <div>\
            ...
            <div style=\{S.subtitle\}>\{household?.name\}</div>\
          </div>\
          <div style=\{\{ display: "flex", alignItems: "center", gap: 12 \}\}>\
            ...
            <button className="ab" onClick=\{() => \{ setSettingsForm(\{ ...settings \}); setShowSettings(true); \}\} style=\{S.settingsBtn\}>\uc0\u9881 \u65039 </button>\
          </div>\
        </div>\
      </div>\
\
        ...
        \{view === "balance" && (\
          <div className="slideup">\
            <div style=\{S.balanceCard\}>\
              <div style=\{S.balanceLabel\}>\{Math.abs(balance) < 0.01 ? "You're all square" : `$\{owedBy\} owes $\{owedTo\}`\}</div>\
...
\
            \{expenses.length > 0 && (\
              <div style=\{\{ marginTop: 28 \}\}>\
                ...
                \{expenses.length > 3 && (\
                  <button onClick=\{() => setView("history")\} style=\{S.viewAll\}>View all \{expenses.length\} expenses \uc0\u8594 </button>\
                )\}\
              </div>\
            )\}\
\
            \{expenses.length === 0 && (\
              <div style=\{S.empty\}>\
                <div style=\{\{ fontSize: 40, marginBottom: 12 \}\}>\uc0\u55357 \u56504 </div>\
        ...
        \{view === "history" && (\
          <div className="slideup">\
            <div style=\{\{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 \}\}>\
              ...
              \{expenses.length > 0 && <button className="ab" onClick=\{exportCSV\} style=\{S.exportBtn\}>\uc0\u11015  Export CSV</button>\}\
            </div>\
\
            \{expenses.length === 0 ? (\
              <div style=\{S.empty\}>\
                <div style=\{\{ fontSize: 40, marginBottom: 12 \}\}>\uc0\u55357 \u56523 </div>\
              ...
              <>\
                ...
                \{settlements.map((s) => (\
                  <div key=\{s.id\} style=\{S.settlementRow\}>\
                    <div style=\{S.settlementIcon\}>\uc0\u10003 </div>\
                    <div style=\{\{ flex: 1 \}\}>\
                      <div style=\{\{ fontSize: 13, color: T.text \}\}>\{s.paid_by\} paid \{s.paid_to\}</div>\
                      <div style=\{\{ fontSize: 11, color: T.muted, marginTop: 2 \}\}>\{formatDate(s.created_at)\} \'b7 \{s.expense_count\} expenses cleared</div>\
                    </div>\
  ...
  return (\
    <div className="er" style=\{S.expenseRow\} onClick=\{() => e.notes && setExpanded(!expanded)\}>\
      ...
      <div style=\{\{ flex: 1, minWidth: 0 \}\}>\
        ...
        <div style=\{S.expenseMeta\}>\{e.paid_by\} paid \'b7 \{splitLabel\}\{showDate && ` \'b7 $\{formatDate(e.created_at)\}`\}</div>\
      ...
      <div style=\{\{ display: "flex", alignItems: "center", gap: 6 \}\}>\
        <div style=\{\{ textAlign: "right" \}\}>\
          ...
          \{e.notes && <div style=\{\{ fontSize: 9, color: T.muted, marginTop: 1 \}\}>\uc0\u55357 \u56541 </div>\}\
        </div>\
        <div style=\{\{ display: "flex", flexDirection: "column", gap: 4 \}\}>\
          <button className="ib ab" onClick=\{(ev) => \{ ev.stopPropagation(); onEdit(e); \}\} style=\{S.editBtn\}>\uc0\u9999 \u65039 </button>\
          <button className="ib ab" onClick=\{(ev) => \{ ev.stopPropagation(); onDelete(e.id); \}\} style=\{S.deleteBtn\}>\'d7</button>\
        </div>\
      </div>\
    </div>\
  );\
